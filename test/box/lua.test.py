...
admin("box.pack('bsil', 84, 29541, 1802444916, 2338318684567380014ULL)")
...
sql("call box.delete(0, 1)")
sql("insert into t0 values (1, 'test box delete')")
admin("box.delete(0, 1)")
admin("box.delete(0, 1)")
sql("insert into t0 values (1684234849, 'test box delete')")
sql("call box.delete(0, 1)")
sql("call box.delete(0, 1684234849)")
sql("call box.delete(0, 1684234849)")
sql("insert into t0 values (1684234849, 'test box delete')")
admin("box.delete(0, 1684234849)")
admin("box.delete(0, 1684234849)")
sql("call box.select(0, 0, 1684234849)")
sql("insert into t0 values (1684234849, 'test box.select()')")
sql("call box.select(0, 0, 1684234849)")
admin("box.select(0, 0, 1684234849)")
admin("box.select(0, 0)")
admin("box.select(0, 1)")
admin("box.select(0)")
sql("call box.replace(0, 1684234849, 'hello', 'world')")
sql("call box.replace(0, 1684234849, 'goodbye', 'universe')")
sql("call box.select(0, 0, 1684234849)")
sql("call box.select(0, 0, 1684234849)")
sql("call box.replace(0, 1684234849)")
sql("call box.select(0, 0, 1684234849)")
sql("call box.delete(0, 1684234849)")
sql("call box.delete(0, 1684234849)")
...
sql("call field_x(0, 1936941424, 1)")
sql("call box.delete(0, 1936941424)")
...
admin("fifomax")
admin("fifo_push(1953719668, 1)")
admin("fifo_push(1953719668, 2)")
admin("fifo_push(1953719668, 3)")
admin("fifo_push(1953719668, 4)")
admin("fifo_push(1953719668, 5)")
admin("fifo_push(1953719668, 6)")
admin("fifo_push(1953719668, 7)")
admin("fifo_push(1953719668, 8)")
admin("fifo_top(1953719668)")
admin("box.delete(0, 1953719668)")
admin("fifo_top(1953719668)")
admin("box.delete(0, 1953719668)")
...
admin("t:next(1684234849)")
admin("t:next(1)")
admin("t:next(t)")
admin("t:next(t:next())")
admin("ta = {} for k, v in t:pairs() do table.insert(ta, v) end")
admin("ta")
admin("t=box.space[0]:replace(1953719668, 'another field')")
admin("ta = {} for k, v in t:pairs() do table.insert(ta, v) end")
admin("ta")
admin("t=box.space[0]:replace(1953719668, 'another field', 'one more')")
admin("ta = {} for k, v in t:pairs() do table.insert(ta, v) end")
admin("ta")
...
admin("box.fiber.resume(f)")
admin("function y() box.fiber.detach() while true do box.replace(0, 1953719668, os.time()) box.fiber.sleep(0.001) end end")
admin("f = box.fiber.create(y)")
admin("box.fiber.resume(f)")
...
sql("call f1('jason')")
sql("call f1('jason', 1, 1953719668, 2, 'stewart')")
lua = """
...
"""
admin("pcall(box.insert, 99, 1, 1953719668)")
